...
public class GameToolBar extends ToolBar {
    ...
    public void refreshText(GameBoardUI gameBoardUI) {
        this.textField.setText("Viral Load on you = "
                + (double) Math.round(gameBoardUI.getGameBoard().getPlayerCar().getViralLoad() * ROUND_OFF_CONSTANT) / ROUND_OFF_CONSTANT);
        this.virusViralLoad.setText("Virus potency =  "
                + (double) Math.round(gameBoardUI.getGameBoard().getCustomCar().getViralLoad() * ROUND_OFF_CONSTANT) / ROUND_OFF_CONSTANT);
    }
    ...
    public void initializeActions(GameBoardUI gameBoardUI) {
        this.start.setOnAction(event -> {
            ...
            alert.setHeaderText("2022 BUMPER CARS");
