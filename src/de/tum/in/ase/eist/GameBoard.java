...
import de.tum.in.ase.eist.car.CustomCar;
...
public class GameBoard {
    ...
    private final CustomCar covidCar;
    ...
    public GameBoard(Dimension2D size) {
        ...
        this.covidCar = new CustomCar(this.size);

        this.covidCar.setInfected(true);
        this.covidCar.setWearingMask(false);

        this.covidCar.setViralLoad(randomGenerator.nextInt(COVID_POTENCY_SEED) + COVID_POTENCY_OFFSET);
    ...
    public void moveCars() {
        ...
        for (Car car : cars) {
            ...
            if (collision.isCrash() && !(car instanceof CustomCar)) {
    ...
    private boolean allCarsCrunched() {
        for (Car car : cars) {
            if (!(car instanceof CustomCar) && !car.isCrunched()) {
                return false;
            }
        }
    ...
    private void printInfection(Car winner, Car loser) {
        if (winner == player.getCar()) {
            System.out.println("\tPlayerCar with a viral load of "
                    + (double) Math.round(winner.getViralLoad() * ROUND_OFF_CONSTANT) / ROUND_OFF_CONSTANT
                    + " and " + (double) Math.round(loser.getViralLoad() * ROUND_OFF_CONSTANT) / ROUND_OFF_CONSTANT + " with a viral load of "
                    + loser.getViralLoad() + " were in contact!!!");
        } else if (loser == player.getCar()) {
            System.out.println("\t" + winner.getClass().getSimpleName() + " with a viral load of "
                    + (double) Math.round(winner.getViralLoad() * ROUND_OFF_CONSTANT) / ROUND_OFF_CONSTANT
                    + " and PlayerCar with a viral load of " + (double) Math.round(loser.getViralLoad() * ROUND_OFF_CONSTANT) / ROUND_OFF_CONSTANT
                    + " were in contact!!!");
        } else {
            System.out.println("\t" + winner.getClass().getSimpleName() + " with a viral load of "
                    + (double) Math.round(winner.getViralLoad() * ROUND_OFF_CONSTANT) / ROUND_OFF_CONSTANT
                    + " and " + loser.getClass().getSimpleName() + " with a viral load of "
                    + (double) Math.round(loser.getViralLoad() * ROUND_OFF_CONSTANT) / ROUND_OFF_CONSTANT
                    + " were in contact!!!");
        }
